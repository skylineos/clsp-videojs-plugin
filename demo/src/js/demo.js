...
function initializeTour () {
  function createVideoPlayer (index, playerOptions) {
    ...
    player.on('changesrc', (event, source) => {
      $container.find('.video-stream .url').text(source.url);
    });
    ...
    // player.on('network-error', function (evt, message) {
    ...
    setInterval(() => {
      player.trigger('changesrc', {
        eid: 'tour-video',
        url: playerOptions.sources[tourCounter % playerOptions.sources.length].src,
      });
